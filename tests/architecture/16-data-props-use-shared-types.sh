...
VIOLATIONS=$(git ls-files '*.tsx' -- src/lib/components/ | xargs grep -n -E ':\s*\{[^}]*(id|created_at|updated_at|user_id)\s*:' 2>/dev/null | grep -v 'import' | grep -v '@/types' || true)

if [ -n "$VIOLATIONS" ]; then
  AVAILABLE=$(grep -E '^export (type|interface)' src/lib/shared/types/index.ts | sed 's/export //' | awk '{print $2}' | tr -d ';' | tr '\n' ' ' | sed 's/ $//')
